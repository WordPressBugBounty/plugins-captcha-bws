	...
	function cptch_admin_init() {
		...
		if ( empty( $bws_plugin_info ) ) {
			$bws_plugin_info = array(
				'id' => '75',
				'version' => $cptch_plugin_info['Version'],
			);
		}
	...
	function cptch_settings() {
		...
		if ( empty( $cptch_options['plugin_db_version'] ) || $cptch_options['plugin_db_version'] != $db_version ) {
			...
			if ( isset( $cptch_options['plugin_option_version'] ) && version_compare( $cptch_options['plugin_option_version'], '5.1.0', '<' ) ) {
				/* Update tables when update plugin and tables changes */
				$wpdb->query( 'RENAME TABLE `' . $wpdb->prefix . 'cptch_whitelist` TO `' . $wpdb->prefix . 'cptch_allowlist`' );
		...
		if ( empty( $cptch_options['plugin_option_version'] ) || $cptch_options['plugin_option_version'] != $cptch_plugin_info['Version'] ) {
			...
			if ( ! isset( $cptch_options['forcibly_enable_time_limit'] ) ) {
				$cptch_options['forcibly_enable_time_limit'] = true;

				foreach( $cptch_options['forms'] as $key => $value ) {
					if ( ! isset( $cptch_options['forms'][ $key ]['enable_time_limit'] ) || false === $cptch_options['forms'][ $key ]['enable_time_limit'] ) {
...
if ( ! function_exists( 'cptch_generate_key' ) ) {
	 ...
	 *
	 * @param int $lenght Length for key.
	 */
	...
	function cptch_allowlist_template() {
		...
		<div class="wrap">
			...
			<?php if ( $bws_hide_premium ) { ?>
				<p>
					<?php
					esc_html_e( 'This tab contains Pro options only.', 'captcha-bws' );
					echo ' ' . sprintf(
						esc_html__( '%1$sChange the settings%2$s to view the Pro options.', 'captcha-bws' ),
			...
			<?php } else { ?>
				<div class="bws_pro_version_bloc">
					...
					<div class="bws_pro_version_tooltip">
						<a class="bws_button" href="https://bestwebsoft.com/products/wordpress/plugins/captcha/?k=9701bbd97e61e52baa79c58c3caacf6d&pn=75&v=<?php echo esc_attr( $cptch_plugin_info['Version'] ); ?>&wp_v=<?php echo esc_attr( $wp_version ); ?>" target="_blank" title="Captcha Pro Plugin"><?php esc_html_e( 'Upgrade to Pro', 'captcha-bws' ); ?></a>
					</div>
				</div>
			<?php } ?>
		</div>
...
if ( ! function_exists( 'cptch_login_check' ) ) {
	 ...
	 * WP login form check
	 *
	 * @param  object $user     User object after login.
...
if ( ! function_exists( 'wpmu_cptch_register_form' ) ) {
	/**
	 * WP register form hooks for multisite
	 *
...
if ( ! function_exists( 'cptch_register_check' ) ) {
	/**
	 * WP register form check
	 *
	 * @param array $error Errors array.
	 */
...
if ( ! function_exists( 'cptch_lostpassword_check' ) ) {
	/**
	 * WP lost password form check
	 *
	 * @param bool $allow Flag for display form.
	 */
...
if ( ! function_exists( 'cptch_comment_post' ) ) {
	...
	function cptch_comment_post( $comment ) {
		...
		if ( is_string( $error ) ) {
			wp_die( wp_kses_post( $error ) . '<br />' . esc_html__( 'Click the BACK button on your browser, and try again.', 'captcha-bws' ) );
		}
...
if ( ! function_exists( 'cptch_display_captcha_shortcode' ) ) {
	 ...
	 * @return string Captcha or empty string.
	 */
...
if ( ! function_exists( 'cptch_display_filter' ) ) {
	 ...
	 * @param  string $form_slug  Form slug for captcha.
...
if ( ! function_exists( 'cptch_verify_filter' ) ) {
	/**
	 * Verify captcha via filter hook
	 *
	 * @param  bool   $allow         (Optional) Content for filter.
	 * @param  string $return_format (Optional) Format for captcha.
	 * @param  string $form_slug     (Optional) Form slug for captcha.
	 * @return string $allow
	 */
...
if ( ! function_exists( 'cptch_check_custom_form' ) ) {
	...
	function cptch_check_custom_form( $allow = true, $return_format = 'string', $form_slug = 'general' ) {
		...
		if ( 'invisible' == $cptch_options['type'] ) {
			 ...
			 */
			if ( empty( $form_slug ) ) {
				$form_slug = isset( $_REQUEST['cptch_form'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['cptch_form'] ) ) : 'general';
			} else {
				$form_slug = esc_attr( $form_slug );
			}
			$form_slug = empty( $form_slug ) || ! array_key_exists( $form_slug, $cptch_options['forms'] ) ? 'general' : $form_slug;
			...
			if ( true === $cptch_options['forms']['general']['enable_session'] && isset( $_SESSION[ $form_slug . '_cptch_time' ] ) && isset( $_SESSION[ $form_slug . '_cptch_result' ] ) && 'slide' !== $cptch_options['type'] && true !== $cptch_options['load_via_ajax'] ) {
				...
				$cptch_result = isset( $_REQUEST['cptch_result'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['cptch_result'] ) ) : null;
			}

			if ( 'slide' == $cptch_options['type'] ) {
				/* Not enough data to verify the CAPTCHA answer */
				if ( empty( $cptch_result ) ) {
					$error_code = 'no_answer';
					/* The CAPTCHA answer is wrong */
				} elseif ( ! cptch_check_slide_captcha_response( $cptch_result ) ) {
				...
				if ( cptch_limit_exhausted( $form_slug ) ) {
					...
					if ( 0 !== strcasecmp( trim( cptch_decode( $cptch_result, $cptch_options['str_key']['key'], $cptch_time ) ), sanitize_text_field( wp_unslash( $_REQUEST['cptch_number'] ) ) ) ) {
						$error_code = 'wrong_answer';
					}
				}
			}
			...
			if ( empty( $error_code ) ) {
				unset( $_SESSION[ $form_slug . '_cptch_result' ] );
				unset( $_SESSION[ $form_slug . '_cptch_time' ] );
...
if ( ! function_exists( 'cptch_display_captcha' ) ) {
	 ...
	 * @param  string $input_name (Optional) Input name for captcha.
	...
	function cptch_display_captcha( $form_slug = 'general', $class_name = '', $input_name = 'cptch_number' ) {
		...
		if ( 'recognition' == $cptch_options['type'] ) {
			...
			$value_encode = cptch_encode( $string, $str_key, $time ) ;
			 ...
			 * for example array( 'words', 'input', 'number' )
			 */
			...
			$value_encode = cptch_encode( $array_math_expression[ $rand_input ], $str_key, $time );
		}
...
if ( ! function_exists( 'cptch_add_scripts' ) ) {
	...
	function cptch_add_scripts() {
		...
		if ( defined( 'DOING_AJAX' ) && DOING_AJAX ) {
			...
			wp_register_script( 'cptch_front_end_script', plugins_url( 'js/front_end_script.js', __FILE__ ), array( 'jquery' ), false, $cptch_options['plugin_option_version'], true );
...
if ( ! function_exists( 'cptch_add_time_limit_notice' ) ) {
	...
	function cptch_add_time_limit_notice() {
		...
		wp_register_script( 'cptch_time_limit_notice_script_' . $cptch_id_postfix, '//', false, null, false );
...
if ( ! function_exists( 'cptch_get_image' ) ) {
	 ...
	 * @param  bool  $use_only_words Flag for use only words.
		...
		if ( -1 == $package_id ) {
			$images = $wpdb->get_results(
				...
				WHERE
					...
					AND
					`{$wpdb->base_prefix}cptch_images`.`number`=%d;",
					implode( ',', $cptch_options['used_packages'] ),
					$value
				),
				ARRAY_N
			);
...
if ( ! function_exists( 'cptch_generate_value' ) ) {
	 ...
	 * Functionality of the captcha logic work for custom form
	 *
	 * @param  int  $value           Id for numbers array.
	 * @param  bool $use_only_words (Optional) Use words for captcha.
	 * @return string $value
	 */
...
if ( ! function_exists( 'cptch_converting' ) ) {
	/**
	 * Convert char to html entity
	 *
	 * @param  int $number_string Captcha string.
...
if ( ! function_exists( 'cptch_encode' ) ) {
	 ...
	 * @param  string $password  Captcha password.
	...
	function cptch_encode( $string, $password, $timestamp ) {
		...
		if ( ! $password ) {
			die( esc_html__( 'Encryption password is not set', 'captcha-bws' ) );
		}
...
if ( ! function_exists( 'cptch_decode' ) ) {
	 ...
	 * @param  int    $timestamp Timestamp.
	 * @return mixed Decoded string or false.
	 */
	function cptch_decode( $string, $key, $timestamp ) {
		/* Check if key for encoding is empty */
		if ( ! $key ) {
			die( esc_html__( 'Decryption password is not set', 'captcha-bws' ) );
		}
		...
		return $error ? false : $decoded_string;
	}
}
...
if ( ! function_exists( 'cptch_limit_exhausted' ) ) {
	...
	function cptch_limit_exhausted( $form_slug = 'general' ) {
		...
		if (
			...
			! empty( $cptch_time ) &&
			$cptch_options['forms']['general']['time_limit'] < time() - absint( $cptch_time )
		) {
			return true;
		} else {
			return false;
...
if ( ! function_exists( 'cptch_front_end_styles' ) ) {
	...
	function cptch_front_end_styles() {
		...
		wp_enqueue_style( 'cptch_stylesheet', plugins_url( 'css/front_end_style.css', __FILE__ ), array(), $cptch_plugin_info['Version'] );
		...
		wp_enqueue_style( "cptch_{$device_type}_style", plugins_url( "css/{$device_type}_style.css", __FILE__ ), array(), $cptch_plugin_info['Version'] );
	}
}

if ( ! function_exists( 'cptch_front_end_scripts' ) ) {
	/**
	 * Add scripts for front pages
	 */
	function cptch_front_end_scripts() {
		...
		if (
			...
			wp_enqueue_script( 'cptch_front_end_script', '//', false, $cptch_plugin_info['Version'], true );
			$args = array(
				'nonce'         => wp_create_nonce( 'cptch', 'cptch_nonce' ),
				'ajaxurl'       => admin_url( 'admin-ajax.php' ),
				'enlarge'       => $cptch_options['enlarge_images'],
...
if ( ! function_exists( 'cptch_admin_head' ) ) {
	/**
	 * Add style and scripts for Dashboard
	 */
	function cptch_admin_head() {
		...
		wp_enqueue_style( 'cptch_icon', plugins_url( 'css/admin_page.css', __FILE__ ), array(), $cptch_plugin_info['Version'] );
...
if ( ! function_exists( 'cptch_reload' ) ) {
	...
	function cptch_reload() {
		...
		$form_slug      = isset( $_REQUEST['cptch_form_slug'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['cptch_form_slug'] ) ) : 'general';
		$class          = isset( $_REQUEST['cptch_input_class'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['cptch_input_class'] ) ) : '';
		$input_name     = isset( $_REQUEST['cptch_input_name'] ) ? sanitize_text_field( wp_unslash( $_REQUEST['cptch_input_name'] ) ) : '';
...
if ( ! function_exists( 'cptch_captcha_is_needed' ) ) {
	/**
	 * Check is captcha need
	 *
	 * @param  string $form_slug       Form slug.
	 * @param  bool   $user_loggged_in Flag for user login.
...
if ( ! function_exists( 'cptch_set_slide_captcha_response' ) ) {
	...
	function cptch_set_slide_captcha_response() {
		...
		$key_saved = $wpdb->insert(
			$wpdb->base_prefix . 'cptch_responses',
			array(
				'response' => $hashed,
				'add_time' => $time,
			)
...
if ( ! function_exists( 'cptch_validate_slide_captcha' ) ) {
	/**
	 * Validate slide captcha response
	 */
...
if ( ! function_exists( 'cptch_delete_expired_responses' ) ) {
	/**
	 * Delete expired response from DB
	 */
