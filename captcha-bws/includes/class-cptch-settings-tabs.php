...
if ( ! class_exists( 'Cptch_Settings_Tabs' ) ) {
	class Cptch_Settings_Tabs extends Bws_Settings_Tabs {
		/**
		 * Forms array for captcha
		 *
		 * @var array
		 */
		private $forms;
		/**
		 * Forms categories for captcha
		 *
		 * @var array
		 */
		private $form_categories;
		/**
		 * Registered forms
		 *
		 * @var array
		...
		public function __construct( $plugin_basename ) {
			...
			$this->forms = array(
				...
				'buddypress_register'       => array(
					'name' => esc_html__( 'Registration form', 'captcha-bws' ),
					'for_pro' => 1,
				),
				'buddypress_comments'       => array(
					'name' => esc_html__( 'Comments form', 'captcha-bws' ),
					'for_pro' => 1,
				),
				'buddypress_group'          => array(
					'name' => esc_html__( 'Create a Group form', 'captcha-bws' ),
					'for_pro' => 1,
				),
				'woocommerce_login'         => array(
					'name' => esc_html__( 'Login form', 'captcha-bws' ),
					'for_pro' => 1,
				),
				...
				'woocommerce_lost_password' => array(
					'name' => esc_html__( 'Forgot password form', 'captcha-bws' ),
					'for_pro' => 1,
				),
				'woocommerce_checkout'      => array(
					'name' => esc_html__( 'Checkout form', 'captcha-bws' ),
					'for_pro' => 1,
				),
				'jetpack_contact_form'      => array(
					'name' => esc_html__( 'Jetpack Contact Form', 'captcha-bws' ),
					'for_pro' => 1,
				),
				'bbpress_new_topic_form'    => array(
					'name' => esc_html__( 'bbPress New Topic form', 'captcha-bws' ),
					'for_pro' => 1,
				),
				'bbpress_reply_form'        => array(
					'name' => esc_html__( 'bbPress Reply form', 'captcha-bws' ),
					'for_pro' => 1,
				),
				'wpforo_login_form'         => array(
					'name' => esc_html__( 'wpForo Login form', 'captcha-bws' ),
					'for_pro' => 1,
				),
				'wpforo_register_form'      => array(
					'name' => esc_html__( 'wpForo Registration form', 'captcha-bws' ),
					'for_pro' => 1,
				),
				'wpforo_new_topic_form'     => array(
					'name' => esc_html__( 'wpForo New Topic form', 'captcha-bws' ),
					'for_pro' => 1,
				),
				'wpforo_reply_form'         => array(
		...
		public function save_options() {
			...
			$this->options['forms']['general']['enable_session']    = ! empty( $_REQUEST['cptch_enable_session'] );

			foreach ( $general_strings as $option ) {
				$value = isset( $_REQUEST[ "cptch_{$option}" ] ) ? sanitize_text_field( wp_unslash( $_REQUEST[ "cptch_{$option}" ] ) ) : '';
			...
			foreach ( $general_arrays as $option => $option_name ) {
				$value = isset( $_REQUEST[ "cptch_{$option}" ] ) && is_array( $_REQUEST[ "cptch_{$option}" ] ) ? array_map( 'sanitize_text_field', array_map( 'wp_unslash', $_REQUEST[ "cptch_{$option}" ] ) ) : array();
		...
		public function tab_settings() {
			...
			<table class="form-table">
				<tr>
					...
					<td>
						<?php
						foreach ( $this->form_categories as $fieldset_name => $fieldset_data ) {
							/**
							...
							*/
							...
							?>
							<p><i><?php echo esc_html( $fieldset_data['title'] ); ?></i></p>
							<br>
							<fieldset id="<?php echo esc_attr( $fieldset_name ); ?>">
								<?php
								foreach ( $fieldset_data['forms'] as $form_name ) {
									...
									<label class="cptch_related">
										...
										echo esc_html( $this->forms[ $form_name ]['name'] );
										?>
										</label>
										<?php
							...
							<tr>
								...
								<td>
									<?php foreach ( $this->form_categories['other_for_pro'] as $fieldset_name => $fieldset_data ) { ?>
										<p><?php echo esc_html( $fieldset_data['title'] ); ?></p>
										<br>
										<fieldset id="<?php echo esc_attr( $fieldset_name ); ?>">
											<?php foreach ( $fieldset_data['forms'] as $form_name => $form_data ) { ?>
												<label>
													<input type="checkbox" disabled="disabled">
													<?php echo wp_kses_post( $this->forms[ $form_data ]['name'] ); ?>
												</label>
												<br />
											<?php } ?>
										</fieldset>
			...
			<table class="form-table">
				...
				foreach ( $options as $key => $data ) {
					...
					?>
					>
						<th scope="row"><?php echo esc_html( ucwords( $data['title'] ) ); ?></th>
						<td>
							<fieldset>
								<?php
								$func = "add_{$data['type']}_input";
								if ( isset( $data['array_options'] ) ) {
									...
									foreach ( $data['array_options'] as $slug => $sub_data ) {
										...
										<label for="<?php echo esc_attr( $id ); ?>">
											...
											if (
												...
												?>
												<input type="radio" id="<?php echo esc_attr( $id ); ?>" name="<?php echo esc_attr( $name ); ?>" disabled="disabled" />
												<?php
											} else {
									...
									if ( isset( $data['inline_description'] ) ) {
										?>
										<span class="bws_info"><?php echo wp_kses_post( $data['inline_description'] ); ?></span>
										<?php
									}
								}
								?>
							...
							<?php if ( isset( $data['block_description'] ) ) { ?>
								<span class="bws_info"><?php echo wp_kses_post( $data['block_description'] ); ?></span>
							<?php } ?>
						</td>
					</tr>
					<?php
				...
				<tr class="cptch_for_math_actions cptch_for_recognition">
					...
					<td>
						...
						<span class="bws_info"><?php echo wp_kses_post( $options[0]['inline_description'] ); ?></span>
					</td>
				</tr>
				<tr class="cptch_time_limit" <?php echo $options[0]['checked'] ? '' : ' style="display: none"'; ?>>
					...
					<td>
						<span class="cptch_time_limit">
							<?php
							$this->add_number_input( $options[1] );
							echo '&nbsp;' . esc_html_e( 'sec', 'captcha-bws' );
							?>
						</span>
					</td>
				</tr>
			...
			foreach ( $this->forms as $form_slug => $data ) {
				...
				?>
				<div class="bws_tab_sub_label cptch_<?php echo esc_attr( $form_slug ); ?>_related_form"><?php echo esc_html( $category_title . $data['name'] ); ?></div>
				<?php if ( 'wp_comments' == $form_slug ) { ?>
						...
						?>
					<table class="form-table cptch_<?php echo esc_attr( $form_slug ); ?>_related_form cptch_related_form_bloc">
						<tr class="cptch_form_option_hide_from_registered"<?php echo esc_html( $style ); ?>>
					...
					<?php if ( ! $this->hide_pro_tabs ) { ?>
						<div class="bws_pro_version_bloc cptch_<?php echo esc_attr( $form_slug ); ?>_related_form cptch_related_form_bloc">
							<div class="bws_pro_version_table_bloc">
								<button type="submit" name="bws_hide_premium_options" class="notice-dismiss bws_hide_premium_options" title="<?php esc_html_e( 'Close', 'captcha-bws' ); ?>"></button>
								<div class="bws_table_bg"></div>
							...
							if ( ! empty( $recommended ) ) {
								...
								<table class="form-table bws_pro_version">
									<tr>
										<td colspan="2">
											<?php
											echo esc_html__( 'You should', 'captcha-bws' ) .
											'&nbsp;<a href="' . esc_url( $link ) . '" target="_blank">' . esc_html( $recommended ) . '&nbsp;' . esc_html( $plugin_name ) . '</a>&nbsp;' .
											( empty( $to_current ) ? '' : esc_html( $to_current ) . '&nbsp;' ) .
			...
			<table class="form-table">
				...
				foreach ( $messages as $message_name => $data ) {
					...
					>
						...
						<td>
							<textarea <?php echo 'id="cptch_' . esc_attr( $message_name ) . '" name="cptch_' . esc_attr( $message_name ) . '"'; ?>><?php echo wp_kses_post( trim( $this->options[ $message_name ] ) ); ?></textarea>
							<?php if ( isset( $data['description'] ) ) { ?>
								<div class="bws_info"><?php echo wp_kses_post( $data['description'] ); ?></div>
							<?php } ?>
						</td>
					</tr>
				<?php } ?>
		...
		private function add_checkbox_input( $args ) {
			if ( ! empty( $args ) && is_array( $args ) ) {
				...
				<input
					...
					value="<?php echo ! empty( $args['value'] ) ? esc_attr( $args['value'] ) : 1; ?>"
					<?php
					echo ( ! empty( $args['disabled'] ) ) ? ' disabled="disabled"' : '';
			...
			<div class="cptch_tabs_package_list">
				...
				foreach ( $package_list as $pack ) {
					...
					<li>
						...
						<span><label for="<?php echo esc_attr( $id ); ?>"><img src="<?php echo esc_url( $packages_url . '/' . $pack['folder'] . '/' . $pack['image'] ); ?>" title="<?php echo esc_attr( $pack['name'] ); ?>"<?php echo esc_html( $styles ); ?>/></label></span>
						<span><label for="<?php echo esc_attr( $id ); ?>"><?php echo esc_html( $pack['name'] ); ?></label></span>
					</li>
				<?php } ?>
				</ul>
			</div>
		 ...
		 *
		 * @access public
		 * @param array $default_options Default options for plugin.
		 */
